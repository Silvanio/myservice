...
import com.myservice.mymarket.model.dto.ImportSectorDTO;
import com.myservice.mymarket.model.dto.ImportStockDTO;
import com.myservice.mymarket.repository.*;
import com.myservice.mymarket.service.ImportFileService;
...
public class ImportFileServiceImpl implements ImportFileService {
    ...
    @Override
    public void uploadHistoricalIBOV(MultipartFile file) {
        logger.info("## - Start LOG uploadHistoricalIBOV");

        Stock stock = stockRepository.findByCode("IBOV");
        ...
        try {
            ...
            String[] HEADERS = {"Date", "Open", "High", "Low", "Close", "Adj Close", "Volume"};
            ...
            for (CSVRecord record : records) {
                ...
                String data = record.get("Date");
                String open = record.get("Open");
                String high = record.get("High");
                String low = record.get("Low");
                String price = record.get("Close");
                String volume = record.get("Volume");
                if (open != null && !open.isBlank() && !open.equalsIgnoreCase("null")) {
                    StockHistoricalData historicalData = new StockHistoricalData();
                    historicalData.setDateQuote(DateUtil.format(data));
                    historicalData.setVolume(getBigDecimalNotNull(volume));
                    historicalData.setOpen(getBigDecimalNotNull(open));
                    historicalData.setLow(getBigDecimalNotNull(low));
                    historicalData.setHigh(getBigDecimalNotNull(high));
                    historicalData.setClose(getBigDecimalNotNull(price));
                    historicalData.setExchange(BVMF);
                    historicalData.setCodeStock(codeStock);
                    if (maxData.before(historicalData.getDateQuote())) {
                        stockHistoricalDataList.add(historicalData);
                    }
                }
            ...
            targetReader.close();
            stockHistoricalDataRepository.saveAll(stockHistoricalDataList);
        } catch (IOException e) {
        ...
        logger.info("## - FIM LOG uploadHistoricalIBOV");
    }

    private BigDecimal getBigDecimalNotNull(String data) {
        if (data == null || data.equalsIgnoreCase("null") || data.isBlank()) {
            data = "0";
        }
    ...
    private void fillStocks(Iterable<CSVRecord> records) {
        ...
        for (CSVRecord record : records) {
            ...
            if (importStockDTO.getCode().contains("TAXA")) {
                continue;
            }
